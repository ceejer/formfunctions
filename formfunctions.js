...
function printDiv(divid, doctitle) {
    var divToPrint = findelement(divid);
    if (divToPrint != null && divToPrint.length > 0) {
        var ttl = "Report Results";
        if (doctitle != null && $.type(doctitle) == "string")
            ttl = doctitle;
        var newWin = window.open("", "prntview", "location=no,width=600, height=825, left=100, top=25");
        newWin.document.title = "Right-Click to Print, or Ctrl+P";
        if (newWin != null && newWin.document != null && newWin.document.body != null)
            $(newWin.document.body).html('');
        var winstyl = "<title>" + ttl + "</title><style>.btn{display:none !important;}select{border:solid thin #ddd !important;-webkit-appearance:none !important;-moz-appearance:none !important;}select:-moz-focusring {color: transparent !important;text-shadow: 0 0 0 #000 !important;}select::-ms-expand { display: none !important; }.pagination>li>a, .pagination>li>span{display:none !important;}table{font-size:10pt;}table.table{border-style:none}.table>tbody>tr>td,.table>tbody>tr>th,.table>tfoot>tr>td,.table>tfoot>tr>th,.table>thead>tr>td,.table>thead>tr>th{padding:2px 3px;white-space:nowrap;font-size:10pt;}.label{border:none;}</style>";
        winstyl = winstyl + "<h3 class='hidden-print alert alert-info alert-dismissable'>Right-Click to Print, or Ctrl+P<button type=\"button\" class=\"close\" data-dismiss=\"alert\">&times;</button></h3>";
        newWin.document.write("<link href=\"/Content/bootstrap.min.css\" rel=\"stylesheet\"><link href=\"/Content/Site.css\" rel=\"stylesheet\" />" + winstyl + divToPrint.html());
