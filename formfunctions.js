...
function subOptions(element, matchesValue, elementid, invert, action) {
  ...
  if (res.length > 0) {
    if (elementid != null && matchesValue!= null) {
      ...
      else if(Object.prototype.toString.call( elementid ) === '[object String]'){
        findElement = $(elementid);
        if(typeof findElement !== "undefined" && findElement.length == 0)
          findElement = $("#"+elementid);
        if(typeof findElement !== "undefined" && findElement.length == 0)
          findElement = $("."+elementid);
      }
    ...
    try{
      if(Object.prototype.toString.call( obja ) === '[object Array]' && Object.prototype.toString.call( objb ) === '[object Array]'){
        ...
        for(var i = 0; i<objb.length; i++){
          if(obja!=null && obja!="" && objb[i]!=null && objb[i]!=""){
            if(contains){
...
/// DESCRIPTION: Prevent form submission if fields are empty or invalid
/// add attribute [check] to user input fields
/// parameters for check are [email],[phone],[zip], or []
/// IMPLEMENTATION: <form onsubmit="return formValid(this)"> - with <input type=submit> 
/// Otherwise have to use for js-based form submissions
/*
var f = document.forms[0];
var valid = true;
  if (f.onsubmit) {
    valid = form.onsubmit.call(f);
  }
  if (valid !== false) {
    f.submit();
  }
...
*/
/// IMPLEMENTATION: <input type=text check=empty /> will not submit form if field is empty
/// IMPLEMENTATION: <input type=text check=email /> will not submit if not in email format or empty
function formValid(formelement) {
  var valid = true;
  var radioname = [];
  $(formelement).find('.forminvalid').remove();
  try {
    for(var index=0;index<formelement.elements.length;index++){
      var elem = $(formelement.elements[index]);
      var needed = elem.attr("check");
      if(!elem.closest(".subOptHide").length){
        if (needed != null && needed.length) {
          if (elem.is('input[type="checkbox"]')) {
            var elemnm = elem.attr("name");
            if(!matches(radioname,elemnm)){
              var errtxt = elem.attr("message");
              radioname.push( elemnm );
              var thisblank = "";
              var checkgroup = $(formelement[elemnm]);
              var checkedgroup = checkgroup.filter(":checked");
              if(checkedgroup !=null && checkedgroup.length)
                thisblank = "";
              else {
                valid = false;
                if(errtxt!=null && errtxt!="")
              ...
              checkgroup.each(function(ind,el){
                if(thisblank == ""){                  
                  $(el).parent().removeClass('chkrdinvalid');
                } else {
                  if(!$(el).parent().hasClass('chkrdinvalid'))
                    $(el).wrap("<span class='chkrdinvalid'>" + thisblank + "</span>");
                }
              });
            }
          }
          if (elem.is('input[type="radio"]')) {
            ...
            if(!matches(radioname,elemnm)){
              ...
              radioname.push( elemnm );
              var thisblank = "";
              var radiogroup = $(formelement[elemnm]);
              var radiochecked = radiogroup.filter(":checked");
              if(radiochecked !=null && radiochecked.length)
                thisblank = "";
              else {
                valid = false;
                if(errtxt!=null && errtxt!="")
              ...
              radiogroup.each(function(ind,el){
                if(thisblank == ""){                  
                  $(el).parent().removeClass('chkrdinvalid');
                } else {
                  if(!$(el).parent().hasClass('chkrdinvalid'))
          ...
          if (elem.is('input[type="text"]') || elem.is('input[type="password"]') || elem.is('input[type="number"]') || elem.is('input[type="date"]') || elem.is('input[type="time"]') || elem.is('input[type="email"]') || elem.is('input[type="range"]') || elem.is('input[type="tel"]')) {
            var thisblank = blank(elem);
            if (thisblank!="") {
              elem.after("<span class='forminvalid'>" + thisblank + "</span>");
              elem.focus();
              valid = false;
            }
          }
        ...
        if(elem.is("select")){
          ...
          elem.prop("checked",elem.prop("defaultChecked"));
        }
      }
    }
  } catch (xc) {
    valid = false;
    //console.log(xc.message);
  }
  return valid;
}
/**********************
...
$(document).ready(function(){
  $('.expandretract').on("keypress",function() {
    var thd = $(this)[0];
  ...
  $('textarea.autoexpand, div.autoexpand, li.autoexpand, p.autoexpand').on("keypress change",function() {
    var thd = $(this)[0];
    var totalHeight = 0, currentHeight=0;
    if(typeof thd.clientHeight!= "undefined")
      currentHeight = thd.clientHeight;
  ...
  $(':text.autoexpand').on("keypress change",function() {
    var thd = $(this);
...
******************************/
  $('textarea.remainder, input.remainder').on("keypress",function(){
    var cnt = 255;
    var wrds = 0;
...
function validateform(f) {
  ...
  if(frmsb!=null && frmsb.length)
    frmsb.hide();
  var valid = formValid(f);
  try {updatesignatures();} catch(e) {}
  if(typeof validatesignatures=="function") {
    var edesc=validatesignatures();
    if(edesc!=null && edesc!=""){
	if(frmsb!=null && frmsb.length)
      frmsb.show();
      return false;
  ...
  if(valid){
    if(f.FUNCTION.value !=null && f.FUNCTION.value =="")
    f.FUNCTION.value ="SAVE";
    f.target = "";
    if(f.CONFIRMSCREEN.value !=null && f.CONFIRMSCREEN.value =="")
    f.CONFIRMSCREEN.value = "SAVECONFIRMREFRESHSUMMARY";
    f.submit();
  } else {
    if(frmsb!=null && frmsb.length)
      frmsb.show();
...
function deleteform(f,dserno,cnfmsg) {
  var functionreturnvalue = null;
  if (dserno!=null && dserno == "") {
  ...
  if(typeof cnfmsg == "undefined" || $.trim(cnfmsg) == "")
    cnfmsg = "Are you sure you want to delete this record?";
  var msgtemp = confirm(cnfmsg);
  if(msgtemp){
...
function quicksave(f) {
  var frmsb = $("#formsubmission");
  if(frmsb!=null && frmsb.length)
    frmsb.hide();
  var sgn = f.SIGNATURE;
  if(sgn!=null && sgn.value != ""){
    alert("Signature not allowed on Quick Save");
    return null;
  }
  var Defaulttarget = f.target;
  f.target = "fvalidate";
...
function printDiv(divid) {
  ...
  if(divToPrint != null && divToPrint.length>0){
    var newWin = window.open("","prntview","toolbar=no,location=no,directories=no,menubar=no,width=600, height=825, left=100, top=25");
...
function ajaxaddfavorites(url, elementid, clnum) {
  var showElement =$('');
  if (typeof elementid !== "undefined") {
    ...
    else if(Object.prototype.toString.call( elementid ) === '[object Object]'){
      showElement = elementid;
    }
  ...
  $.ajax({
    ...
    success: function (data) {
      var getData = $("<div>").html(data);
      var response = getData.find( ".data-response" );
      if(showElement!=null && showElement.length>0){
		if(response!=null && response.length>0)
          showElement.html(response.html());
        else{
          var ajxdata = $("<div />").html(data.replace(/^\s+<\/script>/,""));
          ajxdata.find("script").each(function(i) {
            eval($(this).text());
          });
          ajxdata.dialog({modal:true,width:$("body").width()}).dialog('open');
        }
		
		}
      if(typeof writefavlinks == "function")
        writefavlinks(clnum);

    },
    error: function(jqXHR, textStatus, errorThrown) {
      var diverr = $("<div />").addClass("forminvalid").text(errorThrown);
      if(showElement!=null && showElement.length>0)
        showElement.html(diverr.html());
      return diverr.html();
    }
  });
